    ...
    def _concat_messages(messages):
        ...
        for message in messages:
            if message["role"] == "system":
                ...
                message_text += "Human: " + message["content"].strip() + "\n"
            elif message["role"] == "assistant":
                message_text += "Assistant: " + message["content"].strip() + " " + tokenizer.eos_token
            else:
...
def main(
    ...
    if select_token_level == 'global': 
        ...
        retained_tokens_indices = copy.deepcopy(list(raw_labels))
        for i,j in select_tokens_indices:
            retained_tokens_indices[i][j] = -100
    ...
    final_data_path = label_path + f"discarded_harmful_token_labels_{data_type}.pt"
    torch.save(selected_labels, final_data_path)
    print(f"***discarded Token-level label has been stored in {final_data_path} ***")

    retained_tokens_path = label_path + f"retained_token_labels_{data_type}.pt"
    torch.save(retained_tokens_indices, retained_tokens_path)
    print(f"***retained Token-level label has been stored in {retained_tokens_path} ***")
