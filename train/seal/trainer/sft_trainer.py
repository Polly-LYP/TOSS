...
class SFTTrainer(ABC):
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            loss_mean = 0
            epoch_loss = 0
            for prompts_id_len, inputs, attention_masks, _, label_load in self.train_dataloader:
                ...
                labels = label_load[0].unsqueeze(0).to(inputs.device)

                gpt_loss = self.loss_fn(output.logits, labels)

                # if torch.isnan(gpt_loss):
                #     gpt_loss = torch.tensor(0.0, device=gpt_loss.device, dtype=gpt_loss.dtype,requires_grad=True)
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for prompts_id_len, inputs, attention_masks, _, label_load in eval_dataloader:
                inputs = inputs.squeeze(1).to(torch.cuda.current_device())
                attention_mask = attention_masks.squeeze(1).to(torch.cuda.current_device())
